...
where
	...
	make_class_member_instance_type :: InstanceType SymbolType [ATypeVar] z:{#CheckedTypeDef}  u:{#DclModule}  *VarHeap  *TypeHeaps  *CheckState
	...
	make_default_instance :: Ident SymbolType [ATypeVar] Priority Position DefinedSymbol InstanceType Int
...
where
	...
	substitute_symbol_type st=:{st_vars,st_attr_vars,st_args,st_result,st_context,st_attr_env} environment subst_av_info_ptrs specials type_heaps error
		# (st_vars, st_attr_vars, [st_result : st_args], st_context, st_attr_env, specials, type_heaps, error)
			= instantiateTypes environment st_vars st_attr_vars [st_result:st_args] st_context st_attr_env subst_av_info_ptrs specials type_heaps error
		= ({st & st_vars = st_vars, st_args = st_args, st_result = st_result, st_attr_vars = st_attr_vars,
...
checkFunction fun_def=:{fun_ident,fun_pos,fun_body,fun_type,fun_kind,fun_info={fi_properties}} mod_index fun_index def_level local_functions_index_offset
	...
	# {es_fun_defs,es_calls,es_var_heap,es_expr_heap,es_type_heaps,es_generic_heap,es_dynamics} = e_state
	  ...
	  fi_properties = (if ef_is_macro_fun (fi_properties bitor FI_IsMacroFun) fi_properties) bitor (has_type fun_type)
...
where
	...
	where
		...
		add_type_def td=:{td_pos, td_rhs = AlgConses conses type_ext_ident} new_type_defs new_cons_defs new_selector_defs conversion_table icl_sizes icl_decl_symbols cs
			# (conses,(new_cons_defs,conversion_table,icl_sizes,icl_decl_symbols,cs)) = copy_and_redirect_cons_symbols com_cons_defs td_pos conses (new_cons_defs,conversion_table,icl_sizes,icl_decl_symbols,cs)
			= ([{ td & td_rhs = AlgConses conses type_ext_ident} : new_type_defs],new_cons_defs,new_selector_defs,conversion_table,icl_sizes,icl_decl_symbols,cs)
		add_type_def td new_type_defs new_cons_defs new_selector_defs conversion_table icl_sizes icl_decl_symbols cs
			= ([td : new_type_defs],new_cons_defs,new_selector_defs,conversion_table,icl_sizes,icl_decl_symbols,cs) 
...
//		= undef
	# (nr_of_components, component_numbers)
	  ...
	  (_, expl_imp_infos, dcl_modules, macro_defs, heaps, cs)
			= unsafeFold2St (checkDclComponent components_importing_module_a) (reverse expl_imp_indices) (reverse components)
...
checkDclComponent :: !{![Int]} ![[ExplicitImport]] ![Int]
				!(!Int, !*ExplImpInfos, !*{# DclModule},!*{#*{#FunDef}},!*Heaps,!*CheckState)
			->   (!Int, !*ExplImpInfos, !.{# DclModule},!*{#*{#FunDef}},!.Heaps,!.CheckState)
checkDclComponent components_importing_module_a expl_imp_indices mod_indices
...
//	| False--->("checkDclComponent", mod_indices, size dcl_modules) = undef	
	...
	= case ste_kind of
		STE_ClosedModule
			...
			# (expl_imp_infos, dcl_modules, cs_symbol_table)
					= foldSt (just_update_expl_imp_info components_importing_module_a) mod_indices (expl_imp_infos, dcl_modules, cs.cs_symbol_table)
		...
		STE_Module _
			...
			# imports_ikh
			  ...
			  (afterwards_info, (expl_imp_infos, dcl_modules, macro_defs, heaps, cs))
				= map2St (checkDclModuleWithinComponent dcl_imported_module_numbers is_on_cycle modules_in_component_set components_importing_module_a imports_ikh)
  ...
  where
	...
	just_update_expl_imp_info components_importing_module_a mod_index (expl_imp_infos, dcl_modules, cs_symbol_table)
...
checkDclModuleWithinComponent :: .NumberSet Bool {#.Int} {![.Int]} (IntKeyHashtable SolvedImports) Int *CommonDefs
						   *(*ExplImpInfos,*{#.DclModule},*{#*{#.FunDef}},*Heaps,*CheckState)
	-> ((Int,Int,[FunType]),(*ExplImpInfos,.{# DclModule},.{#.{# FunDef}},.Heaps,.CheckState))
checkDclModuleWithinComponent dcl_imported_module_numbers is_on_cycle modules_in_component_set components_importing_module_a imports_ikh
	...
	= checkDclModule2 dcl_imported_module_numbers components_importing_module_a.[mod_index] imports_ikh is_on_cycle modules_in_component_set False
	...
	where
		...
		where
			build_conversion_table_for_instances_generic_cases_and_type_funs_of_dcl_mod dcl_specials=:{ir_from,ir_to} first_free_index
				...
				= (new_table,n_icl_functions,icl_instances,icl_gencases,icl_type_defs,error)
			...
			build_conversion_table_for_instances dcl_class_inst_index dcl_instances instances_conversion_table_size icl_instances n_icl_functions new_table error
				| dcl_class_inst_index < instances_conversion_table_size
					...
					| size dcl_members == size icl_members
						...
						= build_conversion_table_for_instances (inc dcl_class_inst_index) dcl_instances instances_conversion_table_size icl_instances n_icl_functions new_table error
						# (icl_members,new_table,n_icl_functions,error)
							= build_conversion_table_for_instances_of_members_with_default_instances 0 0 dcl_members icl_members new_table n_icl_functions icl_instance error
						  icl_instances = {icl_instances & [dcl_class_inst_index].ins_members = icl_members}
						= build_conversion_table_for_instances (inc dcl_class_inst_index) dcl_instances instances_conversion_table_size icl_instances n_icl_functions new_table error
					= (new_table,n_icl_functions,icl_instances,error)

			build_conversion_table_for_instances_of_members_with_default_instances dcl_mem_index icl_mem_index dcl_members icl_members new_table n_icl_functions icl_instance error
				| dcl_mem_index < size dcl_members
					# dcl_member = dcl_members.[dcl_mem_index]
			...
			add_possible_default_instance dcl_member icl_mem_index n_icl_functions icl_members
				# new_instance_member = {cim_ident = dcl_member.cim_ident, cim_arity = -1, cim_index = n_icl_functions}
				= { if (i<icl_mem_index)
	  					icl_members.[i]
	  					(if (i==icl_mem_index)
	  						new_instance_member
...
check_module2 mod_ident mod_modification_time mod_imported_objects mod_imports mod_foreign_exports mod_type icl_global_function_range nr_of_cached_modules
	...
	#! n_specified_icl_classes = sizes.[cClassDefs];
	#! n_specified_icl_members = sizes.[cMemberDefs];
	...
	# icl_common = createCommonDefinitions cdefs
	  ...
	  (icl_common,dcl_modules,cs)
		= compare_members_of_exported_classes dcl_conversions n_specified_icl_classes n_specified_icl_members icl_common dcl_modules cs
	...
	# (macro_and_function_local_defs,dcl_modules,cs)
	  ...
	  (instance_types, icl_instances_ranges, icl_common, icl_functions, dcl_modules, hp_var_heap, hp_type_heaps, cs)
			= checkIclInstances icl_instances_ranges icl_common icl_functions dcl_modules hp_var_heap hp_type_heaps cs

	  heaps = { heaps & hp_type_heaps = hp_type_heaps, hp_var_heap = hp_var_heap }

	| not cs.cs_error.ea_ok
		= (False, abort "evaluated error 4 (check.icl)", {}, {}, {}, cs.cs_x.x_main_dcl_module_n,heaps, cs.cs_predef_symbols, cs.cs_symbol_table, cs.cs_error.ea_file, [])
	...
	where
		...
		convert_icl_class_instances2 ci_index next_fun_index ci_a_old ci_a
			| ci_index<size ci_a_old
				...
				= ([],ci_a)
			where
				determine_indexes_of_members [{fun_ident}:members] next_fun_index
					#! (member_symbols, last_fun_index) = determine_indexes_of_members members (inc next_fun_index)
					= ([{cim_ident = fun_ident, cim_index = next_fun_index, cim_arity = -2} : member_symbols], last_fun_index)
				determine_indexes_of_members [] next_fun_index
					= ([], next_fun_index)
...
checkPredefinedDclModule dcl_imported_module_numbers components_importing_module imports_ikh is_on_cycle modules_in_component_set support_dynamics
	...
	= checkDclModule2 dcl_imported_module_numbers components_importing_module imports_ikh is_on_cycle modules_in_component_set support_dynamics
		 mod_ident dcl_common def_macro_indices def_funtypes mod_index expl_imp_info modules macro_defs heaps cs

checkDclModule2 :: !NumberSet ![Int] !(IntKeyHashtable SolvedImports) !Bool !LargeBitvect !Bool
					!Ident *CommonDefs !IndexRange ![FunType] !Index !*ExplImpInfos !*{#DclModule} !*{#*{#FunDef}} !*Heaps !*CheckState
									-> (!(!Int,!Index,![FunType]), !(!*ExplImpInfos,!*{#DclModule},!*{#*{#FunDef}},!*Heaps,!*CheckState))
checkDclModule2 dcl_imported_module_numbers components_importing_module imports_ikh is_on_cycle modules_in_component_set tc_class_defined
